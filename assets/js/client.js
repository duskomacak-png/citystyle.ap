...
function renderShoeViewer() {
  ...
  viewer.innerHTML = `
    ...
    <div class="shoe-viewer-top"><small class="shoe-viewer-code">${escapeHtml(csProductCode(product))}${product.category ? " • " + escapeHtml(product.category) : ""}${imgs.length > 1 ? ` • ${csViewerState.image + 1}/${imgs.length}` : ""}</small><div class="shoe-viewer-headline"><h2>${escapeHtml(product.name || "Patike")}</h2><b>${escapeHtml(csProductPrice(product))}</b></div></div>
