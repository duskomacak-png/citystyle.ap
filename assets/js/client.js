...
function changeImage(dir){const p=products[viewerState.index], imgs=productAllImages(p); if(imgs.length<2)return; viewerState.image=(viewerState.image+dir+imgs.length)%imgs.length; viewerState.zoom=false; renderViewer();}
function changeProduct(dir){viewerState.index=(viewerState.index+dir+products.length)%products.length; viewerState.image=0; viewerState.zoom=false; renderViewer();}
function setViewerImage(e,i){e.stopPropagation(); viewerState.image=i; viewerState.zoom=false; renderViewer();}
function closeOrUnzoom(){if(viewerState?.zoom){viewerState.zoom=false;renderViewer();return;} $('#productViewer')?.remove(); viewerState=null;}
...
Object.assign(window,{showProducts,openViewer,closeOrUnzoom,setViewerImage,shareCurrentProduct,askCurrentProduct,callProfile,showServices,selectService,renderSlots,submitBooking,installInfo});
