...
async function deleteProduct(id) {
  if (stopAdminOwnerPreviewEdit()) return;
  if (!confirm("Obrisati proizvod iz kataloga? Brišu se i sve slike ovog oglasa iz baze i Storage-a.")) return;
  ...
  const urlsToDelete = Array.from(new Set([product?.image_url, ...extraImages.map(x => x.image_url)].filter(Boolean)));

  try { await window.db.from("product_images").delete().eq("product_id", id); } catch (e) {}
  ...
  for (const url of urlsToDelete) {
    try { await window.StorageHelper.deleteImage(url); } catch (e) {}
  }

  await loadProducts();
  window.App.showMessage("Oglas i njegove slike su obrisani.", "success");
}
...
async function saveProduct() {
  ...
  let oldImageForCleanup = null;
  if (file) {
    oldImageForCleanup = image_url;
    image_url = await window.StorageHelper.uploadImage(file, currentSalonId, "product");
  }
  ...
  if (file && oldImageForCleanup && oldImageForCleanup !== image_url) {
    try { await window.StorageHelper.deleteImage(oldImageForCleanup); } catch (e) {}
  }
  hideProductForm(); await loadProducts(); window.App.showMessage("Proizvod je sačuvan.", "success");
}

function productImagePanelNote() {
  return `<div class="product-image-pro-note"><strong>Uputstvo za slike oglasa</strong><p>Dodaj samo čiste slike iz galerije, bez screenshot-a ekrana, bez dugmadi aplikacije i bez status bara telefona. Najlepši prikaz u aplikaciji je <b>9:16</b>, preporuka <b>1080×1920 px</b>. Opis/cenu možeš napraviti kao tekst direktno na slici preko ChatGPT slike, pa ovde dodaješ samo gotovu sliku.</p></div>`;
}
...
async function showProductImages(productId) {
  ...
  const seen = new Set();
  const allImages = [];
  if (product.image_url) { seen.add(product.image_url); allImages.push({ id: "main", image_url: product.image_url, isMain: true }); }
  imgs.forEach(img => {
    if (!img?.image_url || seen.has(img.image_url)) return;
    seen.add(img.image_url);
    allImages.push({ ...img, isMain: img.image_url === product.image_url });
  });

  const modal = document.createElement("div");
  modal.className = "legal-modal-backdrop";
  modal.innerHTML = `<div class="legal-modal-card product-images-modal">
    <div class="legal-modal-head"><h2>Slike oglasa: ${salonEscapeHtml(product?.category || product?.name || 'Proizvod')}</h2><button class="btn btn-dark" onclick="this.closest('.legal-modal-backdrop').remove()">Zatvori</button></div>
    <div class="legal-modal-body product-images-body">
      <div class="card product-images-upload-card">
        <label>Dodaj slike 9:16 iz galerije<input id="product-extra-images" type="file" accept="image/png,image/jpeg,image/webp" multiple></label>
        <p class="field-help product-upload-tip"><strong>Najbolji format:</strong> 9:16, najbolje 1080×1920 px. Možeš dodati više slika po oglasu. Prva/naslovna slika je ona koja se prva prikazuje kupcu.</p>
        <button class="btn btn-primary" onclick="uploadProductExtraImages('${productId}')">Dodaj slike</button>
      </div>
      ${productImagePanelNote()}
      <div class="owner-gallery-grid product-images-grid">${allImages.map((img, idx) => `<div class="card owner-gallery-card product-image-card ${img.isMain ? 'product-image-main' : ''}">
        <div class="product-image-badge">${img.image_url === product.image_url ? 'NASLOVNA' : `SLIKA ${idx + 1}`}</div>
        <img src="${salonEscapeHtml(img.image_url)}" alt="Slika oglasa">
        <div class="product-image-actions">
          ${img.image_url === product.image_url ? `<button class="btn btn-dark btn-small" type="button" disabled>Prva slika</button>` : `<button class="btn btn-primary btn-small" type="button" onclick="setProductMainImage('${productId}','${salonEscapeJs(img.image_url)}')">Postavi kao prvu</button>`}
          <button class="btn btn-danger btn-small" type="button" onclick="deleteProductImage('${img.id || ''}','${productId}','${salonEscapeJs(img.image_url)}')">Obriši</button>
        </div>
      </div>`).join("") || '<p class="muted">Nema slika. Dodaj čistu 9:16 sliku iz galerije.</p>'}</div>
    </div>
  </div>`;
  document.body.appendChild(modal);
}
...
async function uploadProductExtraImages(productId) {
  const files = Array.from(document.getElementById("product-extra-images")?.files || []).slice(0,20);
  if (!files.length) return window.App.showMessage("Izaberi slike iz galerije.", "error");
  ...
  for (const file of files) {
    ...
    await window.db.from("product_images").insert({ product_id: productId, image_url: url, sort_order: 100 });
  }
...
async function setProductMainImage(productId, imageUrl) {
  const { error } = await window.db.from("products").update({ image_url: imageUrl, updated_at: new Date().toISOString() }).eq("id", productId).eq("salon_id", currentSalonId);
  if (error) return window.App.showMessage("Naslovna slika nije promenjena.", "error");
  document.querySelector('.legal-modal-backdrop')?.remove();
  await showProductImages(productId);
  await loadProducts();
...
async function deleteProductImage(imageId, productId, imageUrl) {
  ...
  if (imageId && imageId !== "main") {
    try { await window.db.from("product_images").delete().eq("id", imageId).eq("product_id", productId); } catch(e) {}
  }

  if (product?.image_url === imageUrl) {
    let nextUrl = null;
    try {
      const { data: left } = await window.db.from("product_images").select("image_url").eq("product_id", productId).neq("image_url", imageUrl).order("sort_order", { ascending:true }).order("created_at", { ascending:true }).limit(1);
      nextUrl = left?.[0]?.image_url || null;
    } catch(e) {}
  ...
  try { await window.StorageHelper.deleteImage(imageUrl); } catch(e) {}
  document.querySelector('.legal-modal-backdrop')?.remove();
  await showProductImages(productId);
  await loadProducts();
...
// Kompatibilnost sa starim onclick nazivima iz ranijih verzija
async function deleteProductExtraImage(imageId, productId, imageUrl) { return deleteProductImage(imageId, productId, imageUrl); }
...
Object.assign(window, { editProduct, showAddProductForm, saveProduct, hideProductForm, showProductImages, uploadProductExtraImages, setProductMainImage, deleteProductImage, deleteProductExtraImage, copyProductLink, previewProductAsClient, uploadCoverImage, ensureOwnerPushIsActive });
