...
function updateManifestForSalon(slug, options = {}) {
  ...
  const baseManifest = {
    ...
    start_url: `${getAppBaseUrl()}?${startParam}&pwa_profile=${encodedProfile}&v=v197_safe_owner_panel_mobile_laptop`,
    scope: getAppBaseUrl(),
    display: "standalone",
    background_color: "#0b0b0f",
    theme_color: theme,
...
async function registerPushForSalon(salonId) {
  try {
    ...
    await navigator.serviceWorker.register("/sw.js?v=v197_safe_owner_panel_mobile_laptop", { scope: "/" });
