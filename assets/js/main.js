...
function updateManifestForOwner(options = {}) {
  ...
  const start = `${getAppPath("salon/")}?pwa_owner=1&owner=${encodedKey}&v=v250_clean_product_card_viewer`;
  const baseManifest = {
...
function updateManifestForSalon(slug, options = {}) {
  ...
  const baseManifest = {
    ...
    start_url: `${getAppBaseUrl()}?${startParam}&pwa_profile=${encodedProfile}&v=v250_clean_product_card_viewer`,
    scope: getAppBaseUrl(),
    display: "standalone",
    background_color: "#0b0b0f",
    theme_color: "#0b0b0f",
...
async function registerPushForSalon(salonId, options = {}) {
  try {
    ...
    await navigator.serviceWorker.register("/sw.js?v=v250_clean_product_card_viewer", { scope: "/", updateViaCache: "none" });
    ...
    if (options.showTestNotification) {
      try {
        ...
        await registration.showNotification("CityStyle obaveštenja aktivna", {
          body: "Test radi. Kada klijent zakaže termin, notifikacija stiže ovde.",
        ...
        showMessage("Obaveštenja su uključena. Proveri Android obaveštenja odozgo.", "success");
      } catch (testErr) {
