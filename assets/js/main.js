...
function updateManifestForOwner() {
  const baseManifest = {
    ...
    start_url: `${getAppPath("salon/")}?pwa_owner=1&v=business30hardreset`,
    scope: getAppBaseUrl(),
    display: "standalone",
...
function updateManifestForSalon(slug, options = {}) {
  ...
  const baseManifest = {
    ...
    start_url: `${getAppBaseUrl()}?salon=${encodedSlug}&pwa_profile=${encodedSlug}&v=business30hardreset`,
    scope: getAppBaseUrl(),
    display: "standalone",
    background_color: "#0b0b0f",
    theme_color: theme,
...
async function registerPushForSalon(salonId) {
  try {
    ...
    const registration = await navigator.serviceWorker.register("/sw.js?v=business30hardreset", { scope: "/" });
