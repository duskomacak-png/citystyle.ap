...
function updateManifestForOwner(options = {}) {
  ...
  const start = `${getAppPath("salon/")}?pwa_owner=1&owner=${encodedKey}&v=v246_store_viewer_final`;
  const baseManifest = {
...
function updateManifestForSalon(slug, options = {}) {
  ...
  const baseManifest = {
    ...
    start_url: `${getAppBaseUrl()}?${startParam}&pwa_profile=${encodedProfile}&v=v246_store_viewer_final`,
    scope: getAppBaseUrl(),
    display: "standalone",
    background_color: "#0b0b0f",
    theme_color: "#0b0b0f",
...
async function registerPushForSalon(salonId, options = {}) {
  try {
    ...
    await navigator.serviceWorker.register("/sw.js?v=v246_store_viewer_final", { scope: "/", updateViaCache: "none" });
