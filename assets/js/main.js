...
function updateManifestForOwner(options = {}) {
  ...
  const start = `${getAppPath("salon/")}?pwa_owner=1&owner=${encodedKey}&v=v203_aggressive_icon_safe_zone`;
  const baseManifest = {
...
function updateManifestForSalon(slug, options = {}) {
  ...
  const baseManifest = {
    ...
    start_url: `${getAppBaseUrl()}?${startParam}&pwa_profile=${encodedProfile}&v=v203_aggressive_icon_safe_zone`,
    scope: getAppBaseUrl(),
    display: "standalone",
    background_color: "#0b0b0f",
    theme_color: "#0b0b0f",
...
async function registerPushForSalon(salonId) {
  try {
    ...
    await navigator.serviceWorker.register("/sw.js?v=v203_aggressive_icon_safe_zone", { scope: "/" });
