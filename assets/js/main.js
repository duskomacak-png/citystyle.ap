...
function updateManifestForOwner(options = {}) {
  ...
  const start = `${getAppPath("salon/")}?pwa_owner=1&owner=${encodedKey}&v=v231_white_shop`;
  const baseManifest = {
...
function updateManifestForSalon(slug, options = {}) {
  ...
  const baseManifest = {
    ...
    start_url: `${getAppBaseUrl()}?${startParam}&pwa_profile=${encodedProfile}&v=v231_white_shop`,
    scope: getAppBaseUrl(),
    display: "standalone",
    background_color: "#0b0b0f",
    theme_color: "#0b0b0f",
