...
function updateManifestForSalon(slug, options = {}) {
  ...
  const baseManifest = {
    ...
    start_url: `${getAppBaseUrl()}?${startParam}&pwa_profile=${encodedProfile}&v=v195_radno_vreme_fix`,
    scope: getAppBaseUrl(),
    display: "standalone",
    background_color: "#0b0b0f",
    theme_color: theme,
...
async function registerPushForSalon(salonId) {
  try {
    ...
    await navigator.serviceWorker.register("/sw.js?v=v195_radno_vreme_fix", { scope: "/" });
